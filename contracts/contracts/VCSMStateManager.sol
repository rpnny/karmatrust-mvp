 ...
 * @dev Generated by snarkjs from tier_membership.circom circuit
 */
interface IGroth16Verifier {
    /**
     * @notice Verify a Groth16 proof
     * @param _pA First element of the proof (G1 point)
     * @param _pB Second element of the proof (G2 point)
     * @param _pC Third element of the proof (G1 point)
...
contract VCSMStateManager is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Update state with ON-CHAIN ZK proof verification
     * 
     * @dev THIS IS THE FULLY TRUSTLESS VERSION - "Trust Math, Not Humans"
     * 
     * How it works:
     * 1. User generates ZK proof off-chain (via backend or locally)
     * 2. User submits proof directly to this function
     * 3. Contract calls Groth16Verifier to verify proof ON-CHAIN
     * 4. If valid, state is updated without any trusted intermediary
     * 
     * Security guarantees:
     * - No backend involvement in verification
     * - Mathematical guarantee that user's score falls within claimed tier bounds
     * - Commitment binding prevents proof replay with different states
     * 
     * Gas cost: ~250k-300k gas (due to pairing operations)
     * 
     * @param _pA Proof element A (G1 point)
     * @param _pB Proof element B (G2 point)
     * @param _pC Proof element C (G1 point)
     * @param _pubSignals Public signals [tier, lowerBound, upperBound, commitment]
     * @param _newStateHash New state hash (must match commitment in _pubSignals[3])
     */
    function updateStateWithProof(
        ...
        // Require verifier to be set
        require(zkpVerifier != address(0), "ZKP verifier not configured");
        ...
        uint8 newLevel = uint8(_pubSignals[0]);
        if (newLevel == 0 || newLevel > 5) {
            revert InvalidLevel();
        }
        
        // Verify commitment matches the state hash being claimed
        // _pubSignals[3] is the commitment from the ZK circuit
        bytes32 proofCommitment = bytes32(_pubSignals[3]);
        require(proofCommitment == _newStateHash, "Commitment mismatch");
        
        // ======================================
        // 🔐 ON-CHAIN ZK PROOF VERIFICATION
        // ======================================
        // This is the key difference from updateState()
        // Here we verify the proof ON-CHAIN using the Groth16 verifier
        bool isValid = IGroth16Verifier(zkpVerifier).verifyProof(_pA, _pB, _pC, _pubSignals);
        if (!isValid) {
            revert InvalidProof();
        }
        
        // Proof is valid - update state
        bytes32 oldHash = state.stateHash;
        uint8 oldLevel = state.level;
        ...
        // Compute proof hash for event logging
